...
export default function ChamadosObrasUsers() {
    ...
    useEffect(() => {
        if (!isLoading) {
            const isAuthorized = user && (
                ...
                (user.role === "Técnico" && user.specificApplications?.includes("Chamados Obras"))
            );
            if (!isAuthorized) {
                router.replace("/home");
            }
                        ...
                        <TabsContent key={groupName} value={groupName} className="w-full">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {filteredUsers(user.usersByGroup[groupName])
                                    ...
                                    .map((user) => {
                                        const isPromoted = updatedUsers[user.username] ?? user.specificApplications?.includes("Chamados Obra");
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {!isPromoted && <SelectItem value="promote"><p className="text-slate-700">Instalar Chamados Obra</p></SelectItem>}
                                                        {isPromoted && <SelectItem value="demote"><p className="text-slate-700">Desinstalar Chamados Obra</p></SelectItem>}
                                                        {isPromoted && <SelectItem value="promoted"><p className="text-slate-700">Técnico Chamados Obra</p></SelectItem>}
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                        );
